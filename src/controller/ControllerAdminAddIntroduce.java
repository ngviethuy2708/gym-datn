...
public class ControllerAdminAddIntroduce extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ControllerAdminAddIntroduce() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("load".equals(request.getParameter("type"))){
			RequestDispatcher rd = request.getRequestDispatcher("/admin/addIntroduce.jsp");
			...
			Introduce objIntro = new Introduce(0, name, preview, detail, pictureNew);
			if(mIntro.addItem(objIntro) > 0){
				response.sendRedirect(request.getContextPath() + "/admin/indexIntroduce");
				return ;
			}else{
				response.sendRedirect(request.getContextPath() + "/admin/addIntroduce");
				return ;
			}
		}
	}
